...
def get_booking_details_api(booking_id: int, db: Session = Depends(get_db)):
    ...
    # Get Room ID (assuming single room per booking for simplicity in this view)
    ...
    # Define time range
    start_time = datetime.combine(booking.check_in, datetime.min.time())
    end_time = datetime.combine(booking.check_out, datetime.max.time())
    ...
    if room_id:
        s_requests = db.query(ServiceRequest).filter(
            ...
            ServiceRequest.created_at >= start_time,
            ServiceRequest.created_at <= end_time
    ...
    # 4. Fetch Inventory Usage (Stock Issues to this Room's Location)
    inventory_usage_data = []
    if room_id:
        # Find Location ID for this Room
        from app.models.inventory import Location
        # Assuming we can find location by analyzing location names or having a direct link.
        # Ideally Room has a location_id or IS a location.
        # Currently Room model doesn't explicitly link to Location table in inventory, but let's assume mapped by Name
        ...
        if loc:
            issues = db.query(StockIssueDetail).join(StockIssue).filter(
                ...
                StockIssue.issue_date >= start_time,
                StockIssue.issue_date <= end_time
