...
def _calculate_bill_for_single_room(db: Session, room_number: str):
    ...
    if checkout_request and checkout_request.inventory_data:
        for item_data in checkout_request.inventory_data:
            item_id = int(item_data.get('item_id') or 0)
            ...
            allocated_stock = float(item_data.get('allocated_stock', 0))
            ...
            if is_fixed_asset or is_rentable:
                ...
                if total_qty > 0 or usage_charge > 0 or bad_charge > 0:
                    ...
                    charges.consumables_charges = (charges.consumables_charges or 0) + usage_charge + bad_charge
                    charges.consumables_items.append({
                        "item_id": item_id,
                        "item_name": label,
                        "actual_consumed": total_qty,
                        "complimentary_limit": inv_item.complimentary_limit or 0,
                        "charge_per_unit": (usage_charge + bad_charge) / total_qty if total_qty > 0 else replacement_price,
                        "total_charge": usage_charge + bad_charge
                    })
...
def _calculate_bill_for_entire_booking(db: Session, room_number: str):
    ...
    processed_ids = set()
    seen_names = set() # Avoid duplicates across rooms if names mismatch slightly
    ...
    for checkout_request in checkout_requests:
        if checkout_request.inventory_data:
            for item_data in checkout_request.inventory_data:
                ...
                # 1. Classification (Hardened)
                selling_price = float(inv_item.selling_price or 0.0)
                replacement_price = selling_price if selling_price > 0 else (float(inv_item.unit_price or 0.0) * (1.0 + float(inv_item.gst_rate or 0.0) / 100.0))
                
                is_fixed_asset = inv_item.is_asset_fixed
                ...
                if used_qty > 0 or allocated_stock > 0:
                    if is_rentable:
                        # For rentals, we often store the rental price in a map or use selling price
                        # In entire booking, we'll try to find if it was issued with a price
                        p_price = 0.0
                        if inv_item.id in rental_map:
                            p_price = rental_map[inv_item.id].get("rental_price", 0)
                        usage_charge = max(used_qty, allocated_stock) * (p_price or selling_price)
                    elif is_amenity:
                        if used_qty > max(2, limit):
                            usage_charge = (used_qty - limit) * replacement_price
                    elif (getattr(inv_item, 'is_sellable_to_guest', True) or limit > 0):
                ...
                if is_fixed_asset or is_rentable:
                    ...
                    charges.inventory_usage.append({
                        ...
                        "quantity": display_qty,
                        "unit": inv_item.unit or "pcs",
                        "rental_price": usage_charge / display_qty if (display_qty > 0 and usage_charge > 0) else (selling_price if is_rentable else 0),
                        "rental_charge": usage_charge,
                        "is_rental": True,
                        "is_payable": (usage_charge > 0 or is_rentable),
                    ...
                    if bad_charge > 0:
                        ...
                        charges.asset_damages.append({
                            "item_name": f"{clean_name} (Damaged/Missing)",
                            "replacement_cost": bad_charge,
                            "notes": f"Room {checkout_request.room_number}: Damaged: {damage_qty}, Missing: {missing_qty}"
                        })
                else:
                    # Consumables
                    total_qty = used_qty + missing_qty + damage_qty
    ...
    # Fetch additional stock issues for anything NOT in the audits
        ...
        for issue in stock_issues:
            ...
            for detail in issue.details:
                if detail.item:
                    clean_name = re.sub(r'\s*\(\s*[xX]\d+[^)]*\)', '', detail.item.name).strip()
                    item_key = f"{clean_name.lower()}_{detail.item_id}"
                    if (detail.item_id and int(detail.item_id) in processed_ids) or (item_key in seen_item_keys):
                        continue
                    ...
                    charges.inventory_usage.append({
                        ...
                        "is_payable": u_charge > 0,
                        "notes": f"Issued stock (Room {room_num})"
                    })
                    if u_charge > 0:
                        charges.inventory_charges = (charges.inventory_charges or 0) + u_charge
