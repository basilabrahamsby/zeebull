...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    import sys
    error_msg = f"VALIDATION ERROR in {request.method} {request.url.path}: {exc.errors()}"
    print(error_msg)
    sys.stderr.write(error_msg + "\n")
    
    # Log form data if possible
    try:
        if request.headers.get("content-type", "").startswith("application/x-www-form-urlencoded") or \
           request.headers.get("content-type", "").startswith("multipart/form-data"):
             try:
                 ...
                 # filter out binary/file data for printing
                 safe_form = {k: v for k, v in form.items() if isinstance(v, str)}
                 print(f"Form Data (Text): {safe_form}")
             except:
                 print("Could not parse form data for logging")
    except Exception:
        pass
