...
TABLES_TO_CLEAR = [
    ...
    "inventory_assignments", # Assuming this table exists, add to list if not present before
...
def force_clear_data_v2():
    db = SessionLocal()
    try:
        print("=== COMMENCING AGGRESSIVE DATA PURGE V2 ===")
        ...
        # 2. Reset Statuses (Corrected for Room model)
        print("\n=== RESETTING ENTITY STATUSES ===")
        
        print("  - Resetting ALL Rooms to 'Available'")
        # housekeeping_status and is_clean didn't exist in the model file, so only update status
