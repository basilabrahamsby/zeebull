...
def count_items(db: Session = Depends(get_db), branch_id: int = Depends(get_branch_id), available: bool = None):
    return food_item.get_food_items_count(db, branch_id=branch_id, available=available)
