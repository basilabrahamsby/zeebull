...
def _list_employees_impl(db: Session, current_user: User, skip: int = 0, limit: int = 20):
    ...
    for emp in employees:
        # Check Clocked In status via Python (Robust against SQL nuances)
        # Fetch all logs for this employee where check_out_time might be open
        # We fetch all open logs specific to this employee to be sure
        emp_open_logs = db.query(WorkingLogModel).filter(
            WorkingLogModel.employee_id == emp.id,
            WorkingLogModel.check_out_time == None
        ).all()
        
        is_clocked_in = len(emp_open_logs) > 0
        
        current_status = "Off Duty"
        if emp.id in leave_map:
            current_status = f"On Leave"
        elif is_clocked_in:
            current_status = "On Duty"
            
        # Create dict to match Pydantic schema
        emp_dict = {
            "id": emp.id,
            "name": emp.name,
            "role": emp.role,
...
def get_employee_status_overview(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    clocked_in_ids = {
        ...
        .filter(WorkingLogModel.check_out_time.is_(None))
        .all()
    }
