...
def update_item_cost_wac(db: Session, item_id: int, new_quantity: float, new_unit_price: float, is_cancellation: bool = False):
    """
...
def update_purchase_status(db: Session, purchase_id: int, status: str):
    ...
    if old_status.lower() != "received" and status.lower() == "received":
        ...
        for detail in purchase.details:
            # Update global stock and cost using WAC
            update_item_cost_wac(db, detail.item_id, detail.quantity, float(detail.unit_price))
