...
with engine.connect() as conn:
    print("ALL USERS WITH 'alphi' IN EMAIL OR NAME:")
    ...
    print("\nALL EMPLOYEES:")
    res = conn.execute(text("SELECT e.id, e.name, e.user_id, e.role FROM employees e;"))
    for row in res:
        print(f"  ID: {row[0]} | Name: {row[1]} | UserID: {row[2]} | Role: {row[3]}")
